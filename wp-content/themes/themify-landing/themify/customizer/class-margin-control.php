		...
		foreach ( $sides as $side => $side_label ) : ?>
			<div class="themify-customizer-brick <?php echo $first ? 'useforall' : 'component'; ?>">
				...
				<?php if ( 'padding' == $dimension_type ) : ?>
					<label for="<?php echo esc_attr( $id ); ?>" class="dimension-row-label <?php echo $first ? 'same-label' : ''; ?>" <?php echo $first ? 'data-same="' . esc_attr( $property ) . '" data-notsame="' . esc_attr( $side_label ) . '"' : ''; ?>><?php echo esc_html( $side_label ); ?></label>
				<?php endif; ?>
