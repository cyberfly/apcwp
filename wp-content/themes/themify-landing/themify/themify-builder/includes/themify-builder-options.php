...
if ( ! function_exists( 'themify_builder_styling_field' ) ) {
	...
	function themify_builder_styling_field( $styling ){
		switch ($styling['type']) {
			...
			case 'text':
				$html = '<input id="' . esc_attr( $styling['id'] ) . '" name="' . esc_attr( $styling['id'] ) . '" type="text" value="" class="' . esc_attr( $styling['class'] ) . ' tfb_lb_option">';
				$html .= isset( $styling['description'] ) ? wp_kses_post( $styling['description'] ) : '';
				echo $html;
			break;

			case 'textarea':
				$value = isset( $styling['value'] ) ? $styling['value'] : '';
				$html = '<textarea id="' . esc_attr( $styling['id'] ) . '" name="' . esc_attr( $styling['id'] ) . '" class="' . esc_attr( $styling['class'] ) . ' tfb_lb_option">' . esc_textarea( $value ) . '</textarea>';
				$html .= isset( $styling['description'] ) ? wp_kses_post( $styling['description'] ) : '';
				echo $html;
			break;
