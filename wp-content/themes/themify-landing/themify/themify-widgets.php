...
class Themify_Social_Links extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( is_array( $field_ids ) || is_object( $field_ids ) ) {
			...
			$out .= '<ul class="social-links ' . esc_attr( $orientation ) . '">';
		...
		function widget( $args, $instance ) {
			...
			if($loop) {
				...
				foreach ($loop as $post) {
					...
					if ( $show_thumb ) {
						$html .= themify_get_image( 'ignore=true&w=' . $instance['thumb_width'] . '&h=' . $instance['thumb_height'] . '&before=<a href="' . esc_url( get_permalink() ) . '">&after=</a>&class=post-img' );
					}

					if( !$hide_title ){
						$html .= '<a href="' . esc_url( get_permalink() ) . '" class="feature-posts-title">' . get_the_title() . '</a>';
					}
					...
					if ( $show_excerpt ) {
						...
						$html .= '<span class="post-excerpt">' . wp_kses_post( $the_excerpt ) . '</span>';
					}
