...
function themify_theme_header_options( $data = array() ) {
	...
	foreach ( $header_design_options as $option ) {
...
/**
 * Markup for module with controls for footer elements
 * @param array $data
 * @return string
 */
function themify_theme_footer_options( $data = array() ) {
	/**
	...
	$html .= sprintf('<p><label for="%1$s"><input type="checkbox" id="%1$s" name="%1$s" %2$s /> %3$s</label></p>',
		$key . 'footer',
		checked( themify_get( $key . 'footer' ), 'on', false ),
		__('Exclude Footer.', 'themify')
	);
...
function themify_related_posts( $data = array() ) {
	...
	$html .= '<p>
				...
				<select name="' . $key . '">' . themify_options_module( $options, $key . '' ) . '
				</select>
			</p>';
	...
	$html .= '<p>
				...
				<input type="text" name="' . $key . '_entries" value="' . $number . '">
			</p>';
